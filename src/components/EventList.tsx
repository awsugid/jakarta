  ...
  return (
    <section className={hideHeader ? "" : "py-24 bg-muted/30"} id="events">
      <div
        className={
          hideHeader
            ? "container mx-auto max-w-7xl px-4 md:px-6 py-12"
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {events.map((event) => {
            ...
            return (
              <Card
                key={event.id}
                className="group relative flex flex-col justify-between border-border/50 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-primary/20 bg-card"
              >
                {event.data.image ? (
                  ...
                  <div className="aspect-video w-full rounded-t-xl bg-muted/30 flex items-center justify-center">
                    <Calendar className="h-8 w-8 text-muted-foreground/30" />
                  </div>
                )}
                <CardHeader className="pt-3 space-y-4">

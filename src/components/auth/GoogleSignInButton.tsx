...
export function GoogleSignInButton({
  ...
  useDialog = true,
  dialogTitle = "Sign In",
  dialogDescription = "Sign in to your AWS Community Jakarta account to continue.",
}: GoogleSignInButtonProps) {
  const [buttonId] = useState(() => `google-signin-${Math.random().toString(36).substring(2, 11)}`);
  ...
  // When the modal opens (or if we aren't using a dialog), render the button
  useEffect(() => {
    const shouldRender = useDialog ? open && ready : ready;
    
    if (shouldRender && window.google?.accounts?.id) {
      // Small timeout to ensure container is mounted in the DOM
          ...
          try {
            ...
            if (onSignIn) {
              (window as any).__gsiOnSuccess = (user: AuthUser | null) => {
                if (useDialog) setOpen(false); // Close modal on success
                onSignIn(user);
              };
            } else {
              (window as any).__gsiOnSuccess = () => {
                if (useDialog) setOpen(false);
              };
            }
          } catch (err) {
  ...
  }, [open, ready, buttonId, onSignIn, useDialog]);
  ...
  const NativeButton = (
    <div className="flex flex-col items-center justify-center w-full min-h-[44px]">
      {ready ? (
        <div id={buttonId} className="transition-opacity opacity-100 inline-block" />
      ) : (
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] flex flex-col items-center py-10 z-[200]">
